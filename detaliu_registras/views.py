...
def convert_network_path_to_url(network_path):
    """
    Priima tinklo kelią (gali būti None) ir grąžina saugią HTTP/URL formą arba None.
    """
    if not network_path:  # None, '' ir pan. atvejai
        return None

    # jei jau yra http/https – paliekam
    if isinstance(network_path, str) and (network_path.startswith('http://') or network_path.startswith('https://')):
        return network_path

    # konversija iš \\server\folder\file į /server/folder/file (arba savo logika)
    http_path = str(network_path).strip('\\').replace('\\', '/')
    return http_path
